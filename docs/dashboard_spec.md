...
- **Symmetric Selection (v1.4)**: Every dashboard supports an **Application** dropdown. Selections for Application, User, and Session are preserved across the entire suite (except **Agent Home**, which completely decouples from session selections to ensure fleet-wide visibility). Summary dashboards use `allValue` placeholders (e.g., `Select_Application`, `Select_User`) to ensure that "All" states propagate correctly to forensic dashboards without passing empty strings.
